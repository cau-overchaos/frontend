...
export default function SearchPopup(props: Props) {
    ...
    const handleAutocomplete: KeyboardEventHandler<HTMLInputElement> = (evt) => {
        ...
        if (evt.key === "ArrowDown" && autocompletes.length > 0) {
            let newIndex = selectedAutocompleteIndex === null ? 0 : selectedAutocompleteIndex + 1;
            ...
            let newIndex = selectedAutocompleteIndex === null ? autocompletes.length - 1 : selectedAutocompleteIndex - 1;
            if (newIndex >= 0) {
                setSelectedAutocompleteIndex(newIndex);
            } else {
                setSelectedAutocompleteIndex(null);
            ...
            if (selectedAutocompleteIndex === null) {
                ...
                props.onAutocompleteRequest(autocompletes[selectedAutocompleteIndex])
                    .then(setAutocompletes)
                    .catch(() => {setAutocompletes([]);})
                    .then(() => {
                        setSelectedAutocompleteIndex(null);
                    })
            }
            ...
            props.onAutocompleteRequest(value)
                .then(setAutocompletes)
                .catch(() => {setAutocompletes([]);})
                .then(() => {
                    setSelectedAutocompleteIndex(null);
    ...
    return <div className={classNames(styles.background, props.active && styles.active)} onClickCapture={(evt) => {
        if ((evt.target as HTMLElement).tagName !== "INPUT")
            props.onCloseButtonClick();
    }}>
        <div className={styles.search}>
            <input type="text" value={query} onChange={(evt) => setQuery(evt.target.value)} className={styles.input} onKeyUp={handleAutocomplete} placeholder='검색어를 입력하세요...' />
